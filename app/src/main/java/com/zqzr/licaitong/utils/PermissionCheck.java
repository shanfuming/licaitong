...
import com.zqzr.licaitong.view.CustomDialog;
...
public class PermissionCheck {
    ...
    private void initApp(int requestCode) {
        if (requestCode == REQUEST_CODE_STORAGE || requestCode == REQUEST_CODE_ALL) {
            File filePath = new File(BaseParams.ROOT_PATH);
            if (!filePath.exists()) {
                filePath.mkdirs();
            }
        }
    ...
    private void showAskDialog(final Activity activity) {
        new CustomDialog.Builder(activity).setMessage(R.string.permission_context)
